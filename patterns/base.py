...
class BasePattern:
    ...
    """ This is the base class for all Candlestick patterns, note
     ...
     The Base Class accepts only one parameter which must be a Pandas DataFrame or Numpy Array
     
     Numpy array >>>> 
         Must be a Numpy array with a shape of (?,4) and order >>>>>>>  open,high,low,close
    ...
    def __init__(self,array_x):
        try:
            ...
            try:
                ...
                print('Values passed did not meet requirement, It must be a Dataframe or Numpy Array')
                raise AssertionError('Could not determine if the value was a valid array')
